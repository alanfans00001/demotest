...
$.fn.serializeOrders1 = function() {
	var o="";
	var a = this.serializeArray();
	$.each(a, function() {	
		var index=this.name.indexOf("^");
		var val=this.name.substring(index+1);	
		if(this.value=="" || this.value=="N"){			
...
$.fn.serializeObjectEx = function(e) {
	var o = {};
	var a = this.serializeArray();
	$.each(a, function() {
		if(checkObj(e,this.name)){
			if (o[this.name]) {
				if (!o[this.name].push) {
					o[this.name] = [o[this.name]];
				}
