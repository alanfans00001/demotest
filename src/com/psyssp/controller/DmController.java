...
public class DmController implements ApplicationContextAware, DisposableBean {
	// 日志
	private static Logger logger = Logger.getLogger(DmController.class);
	...
	public void setApplicationContext(ApplicationContext applicationContext) {
		DmController.applicationContext = applicationContext;
	}

	public void destroy() throws Exception {
		DmController.clearHolder();
	}
	 ...
	 * @throws Exception
	 */
	protected void saveOrUpdate(HttpServletRequest request, DmModel m) throws Exception {
		SimpleDateFormat sdf = new SimpleDateFormat(Utils.DATE_FORMAT);
		String date = sdf.format(new Date());
		if (null == m.getCreatedate() || "".equals(m.getCreatedate())) {
			m.setCreatedate(date);
			m.setCreaterno(getUserSession(request).getUserno());
		}
		m.setModifiedate(date);
		m.setModifierno(getUserSession(request).getUserno());
	}
	
	/**
	 * 物理删除时，对删除时间、删除人两个公共字段的处理
	 * @param request
	 * @param m
	 * @throws Exception
	 */
	protected void forbidden(HttpServletRequest request, DmModel m) throws Exception {
		SimpleDateFormat sdf = new SimpleDateFormat(Utils.DATE_FORMAT);
		String date = sdf.format(new Date());
		m.setForbiddendate(date);
		m.setForbiddenerno(getUserSession(request).getUserno());
	}
	...
	public Object getListPage(HttpServletRequest request, DmService service, Class clazz) throws Exception {
		// 获取前台的分页信息
		...
		if (null != pageFilter) {
			map.put(DmMapper.MAP_KEY_M, om.readValue(pageFilter.toString(), clazz));
		}
		// 设置查询条件的当前页码和每页显示数量
		...
		if (null != pageConfig.optString("orderby", "") && !"".equals(pageConfig.optString("orderby", ""))) {
			map.put(DmMapper.MAP_KEY_ORDERBY, pageConfig.optString("orderby"));
		}
		if (null != pageConfig.optString("order", "") && !"".equals(pageConfig.optString("order", ""))) {
			map.put(DmMapper.MAP_KEY_ORDER, pageConfig.optString("order"));
		}
		// 获取分页的查询数据
		List<DmModel> list = service.findListPage(map, pageInfo);
		...
		pager.setField(null == map.get(DmMapper.MAP_KEY_ORDERBY) ? "" : map.get(DmMapper.MAP_KEY_ORDERBY).toString());
		pager.setSort(null == map.get(DmMapper.MAP_KEY_ORDER) ? "" : map.get(DmMapper.MAP_KEY_ORDER).toString());
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Object getObjectListPage(HttpServletRequest request, DmService service, Class clazz) throws Exception {
		// 获取前台的分页信息
		String str = request.getParameter("page");
		if (null == str || "".equals(str)) {
			return null;
